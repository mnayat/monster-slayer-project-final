                                ...
                                <vue-scrollbar class="dungeons-scrollbar" ref="ScrollBar">
                                    <div class="dungeon-tiles-container center">
                                        <div class="dungeon-tile"
                                            ...
                                            :style="{ 'background-image': 'url(' + getDungeonImg(dungeon.image) + ')'}"
                                            @click="getDungeonDetails(dungeon._id)">
                                            <img v-show="dungeon.locked" class="locked-overlay" :src="getDungeonImg('locked')"/>                                    </div>
                                    </div>
                                </vue-scrollbar>
                            </div>
                        </div>
                    ...
                    <div class="dungeon-details-container">
                        <div class="container">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 center">{{ selectedDungeon.recommendedLevel }}</div>
                            </div>
                            <div class="row nomargin">
                                ...
                                <div class="encounter-list col-sm-12 row nomargin nopadding"
                                    ...
                                    >
                                    <div v-if="!enemy.boss" class="col-sm-3 offset-sm-1 nopadding bold">
                                            {{ enemy.name }}</div>
                                    <div v-if="!enemy.boss" class="col-sm-8 nopadding">
                                        ...
                                        <img class="svg-icon svg-fill" :src="getEquipmentImage(drop.name)">
                                        {{ drop.name }}
                                        </div>
                                            <div v-show="enemy.drops.length == 0" class="dungeon-item col-sm-7 nopadding offset-sm-1">None</div>
                                        </div>
                                </div>
                            </div>
                            <div class="row nomargin">
...
export default {
    ...
    created() {
        this.$store.dispatch(characterActions.getDungeons, this.$store.state.accountModule.player.accountId).then(res => {
    ...
    methods: {
        ...
        getDungeonDetails(id){
            this.selectedDungeon = this.dungeons.filter(function(d) { return d._id == id; })[0];
            this.enemies = this.selectedDungeon.enemies;
        },
